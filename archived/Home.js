...
export default function Index() {
  ...
  /* Auto-shift client logo carousel every 5 seconds */
  useEffect(() => {
    const interval = setInterval(() => {
      setClientPageIndex((prev) => (prev + 1) % 3);
    }, 5000);
    return () => clearInterval(interval);
  }, []);
          ...
          >
            ...
            <div className="client-carousel-container">
              <div 
                ...
                style={{
                  transform: `translateX(-${clientPageIndex * 100}%)`,
                }}
              >
                {/* We create sets of 6 for each page (3 pages for 18 logos) */}
                {[0, 1, 2].map((page) => (
                  <div 
                    key={page}
                    style={{
                  ...
                  >
                    {clientLogos.slice(page * 6, (page * 6) + 6).map((logo, i) => (
                      <div 
                        key={`${page}-${i}`} 
              ...
              >
                {[0, 1, 2].map((idx) => (
                  <button
                    key={idx}
                    onClick={() => setClientPageIndex(idx)}
                    onMouseEnter={() => setIsHoveringNav(true)}
                    onMouseLeave={() => setIsHoveringNav(false)}
                    style={{
                      width: idx === clientPageIndex ? "32px" : "12px",
                      height: "12px",
                      borderRadius: "6px",
                      background: idx === clientPageIndex 
                        ? "linear-gradient(90deg, #89cff0, #8b5cf6)" 
                        : "rgba(255, 255, 255, 0.15)",
                      border: "none",
                      cursor: "pointer",
                      transition: "all 0.6s cubic-bezier(0.16, 1, 0.3, 1)",
                      boxShadow: idx === clientPageIndex 
                        ? "0 0 15px rgba(137, 207, 240, 0.4)" 
                        : "none",
                    }}
                    title={`View set ${idx + 1}`}
                  />
                ))}
              </div>
            </div>
